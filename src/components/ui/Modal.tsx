...
export function Modal({ open, onClose, title, children, size = 'md', footer }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      {/* Dialog */}
      <div className={cn(
        'relative bg-white rounded-3xl shadow-2xl shadow-slate-950/20 ring-1 ring-slate-200/70 w-full animate-fade-in',
        sizes[size],
        ...
        {/* Footer */}
        {footer && (
          <div className="flex-shrink-0 px-7 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50/80 rounded-b-3xl">
            {footer}
          </div>
        )}
      </div>
