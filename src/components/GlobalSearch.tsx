...
export function GlobalSearch({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 px-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-soft w-full max-w-xl animate-fade-in overflow-hidden">
        {/* Input */}
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
          ...
          <input
            ...
            className="flex-1 text-sm bg-transparent outline-none placeholder-slate font-medium"
            aria-label="Recherche globale"
            aria-autocomplete="list"
            aria-activedescendant={activeIndex >= 0 ? `search-result-${activeIndex}` : undefined}
          />
          <button onClick={onClose} className="btn-icon bg-canvas-a" aria-label="Fermer">
            <X size={16} />
          </button>
        </div>
        ...
        <div ref={listRef} className="max-h-80 overflow-y-auto scrollbar-thin py-2" role="listbox">
          {loading && (
            <div className="px-4 py-6 text-center text-sm text-slate font-medium">Recherche…</div>
          )}
          {!loading && query.length >= 2 && results.length === 0 && (
            <div className="px-4 py-6 text-center text-sm text-slate font-medium">
          ...
          {results.map((r, i) => (
            <button
              ...
              className={`w-full flex items-start gap-3 px-5 py-3 text-left transition-colors
                ${i === activeIndex ? 'bg-blue-50' : 'hover:bg-canvas-a'}`}
            >
              <span className={`mt-0.5 flex-shrink-0 ${i === activeIndex ? 'text-blue-700' : 'text-slate'}`}>
                {icons[r.type]}
              </span>
              <div className="min-w-0">
                <p className={`text-sm font-bold truncate ${i === activeIndex ? 'text-blue-700' : 'text-graphite'}`}>
                  {r.label}
                </p>
                <p className="text-xs text-slate truncate font-medium">{r.sublabel}</p>
              </div>
              <span className="ml-auto text-xs text-slate-light capitalize font-semibold">{r.type}</span>
            </button>
          ))}
        </div>

        <div className="px-5 py-3 border-t border-gray-100 text-xs text-slate font-medium flex gap-4">
          <span>↑↓ naviguer</span>
