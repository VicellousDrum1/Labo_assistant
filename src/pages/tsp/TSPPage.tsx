...
const IMPORT_COLUMNS_TSP: ImportColumn[] = [
  { key: 'nom_prenoms',    header: 'Nom & Prénoms',   required: true },
  { key: 'matricule',      header: 'Matricule',        required: false },
  { key: 'fonction',       header: 'Fonction',         required: false },
  { key: 'societe_entite', header: 'Société / Entité', required: false },
  { key: 'exploitation',   header: 'Exploitation',     required: false },
  { key: 'numero_puce',    header: 'N° Puce',          required: false },
  { key: 'operateur',      header: 'Opérateur',        required: false },
    ...
    { key: 'actions', header: '', className: 'w-20 text-right',
      render: r => isAssistant ? (
        <div className="flex justify-end gap-1">
          ...
          {r.actif
            ? <button onClick={() => setDeactivateTarget(r)} className="btn-icon text-danger-500 hover:text-danger-700 hover:bg-danger-50"><PowerOff size={14} /></button>
            : <button onClick={() => toggleActif(r, true)} className="btn-icon text-ok-500 hover:text-ok-700 hover:bg-ok-50"><RotateCcw size={14} /></button>
          }
        </div>
      ) : null
    },
    ...
    <div className="space-y-4 animate-fade-in">
        ...
        <form id="tsp-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              {errors.nom_prenoms && <p className="mt-1 text-xs text-danger-700">{errors.nom_prenoms.message}</p>}
            </div>
            <div>
      ...
      <ImportExcel
        ...
        onImport={async (rows) => {
          ...
          const { created, errorsByIndex } = await batchInsert<{ id_tsp: string }>('tsp', payloads, 'id_tsp')
          ...
          const auditEntries = created
            ...
            .filter(({ c }) => c)
            .map(({ c, row }) => ({
              utilisateur: user?.id ?? null, action: 'IMPORT', table_concernee: 'tsp',
              id_enregistrement: c!.id_tsp, nouvelles_valeurs: row,
            }))
