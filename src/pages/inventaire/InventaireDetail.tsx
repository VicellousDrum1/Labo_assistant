...
export function InventaireDetail() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 animate-fade-in">
      ...
      {!inv.actif && (
        <div className="p-4 bg-danger-50 border border-danger-100 rounded-xl text-sm text-danger-700 flex items-center gap-2">
          <PowerOff size={15} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Section title="Informations générales" icon={Laptop}>
          ...
          <DetailRow label="Statut" value={inv.actif
            ? <span className="badge bg-ok-100 text-ok-700">Actif</span>
            : <span className="badge bg-danger-100 text-danger-700">Désactivé</span>} />
        </Section>
            ...
            {logs.map(log => (
              <div key={log.id_log} className="flex gap-3 text-sm py-2 border-b border-gray-50 last:border-0">
                ...
                <span className={`badge text-xs flex-shrink-0 ${
                  log.action === 'CREATION' ? 'bg-ok-100 text-ok-700'
                  : log.action === 'DESACTIVATION' ? 'bg-danger-100 text-danger-700'
