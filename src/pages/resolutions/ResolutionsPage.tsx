    ...
    { key: 'actions', header: '', className: 'w-20 text-right',
      render: r => isAssistant ? (
        <div className="flex justify-end gap-1">
          ...
          <button onClick={() => setDeleteTarget(r)} className="btn-icon text-red-400 hover:text-red-600 hover:bg-red-50"><Trash2 size={14} /></button>
        </div>
      ) : null
    },
  ]
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="page-header">
        ...
        {isAssistant && <button onClick={() => { setEditing(null); reset({ probleme: '', solution_trouvee: '', observation: '', categorie: '', mots_cles: '' }); setModalOpen(true) }} className="btn-primary text-sm gap-1.5"><Plus size={14} />Nouvelle solution</button>}
      </div>
      ...
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); reset() }}
        ...
        <form id="res-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            {errors.probleme && <p className="mt-1 text-xs text-red-600">{errors.probleme.message}</p>}
          </div>
          <div>
            ...
            {errors.solution_trouvee && <p className="mt-1 text-xs text-red-600">{errors.solution_trouvee.message}</p>}
          </div>
