...
const IMPORT_COLUMNS_DSIM: ImportColumn[] = [
  { key: 'site',           header: 'Site',            aliases: ['Localisation', 'Emplacement'], required: false },
  { key: 'utilisateur',    header: 'Utilisateur',     aliases: ['Nom utilisateur', 'Responsable'], required: false },
  { key: 'societe',        header: 'Société',         aliases: ['Societe', 'Entité'], required: false },
  { key: 'adresse_routeur',header: 'Adresse routeur',  required: false },
  { key: 'numero_sim1',    header: 'N° SIM 1',        aliases: ['SIM 1', 'Numéro SIM 1'], required: false },
  { key: 'operateur_sim1', header: 'Opérateur SIM 1', aliases: ['Operateur SIM 1'], required: false },
  { key: 'numero_sim2',    header: 'N° SIM 2',        aliases: ['SIM 2', 'Numéro SIM 2'], required: false },
  { key: 'operateur_sim2', header: 'Opérateur SIM 2', aliases: ['Operateur SIM 2'], required: false },
  { key: 'observation',    header: 'Observation',     aliases: ['Observations', 'Commentaire'], required: false },
]
    ...
    { key: 'actions', header: '', className: 'w-20 text-right',
      render: r => isAssistant ? (
        <div className="flex justify-end gap-1">
          ...
          {r.actif
            ? <button onClick={() => setDeactivateTarget(r)} className="btn-icon text-red-400 hover:text-red-600 hover:bg-red-50" title="Désactiver"><PowerOff size={14} /></button>
            : <button onClick={() => toggleActif(r, true)} className="btn-icon text-green-500 hover:text-green-700 hover:bg-green-50" title="Réactiver"><RotateCcw size={14} /></button>
          }
        </div>
      ) : null
    },
          ...
          for (let i = 0; i < rows.length; i++) {
            try {
              const { data: created, error } = await supabase
                .from('site_dualsim').insert({ ...rows[i], actif: true }).select('id_dualsim').single()
              if (error) throw error
              await logAudit({ action: 'IMPORT', table_concernee: 'site_dualsim', id_enregistrement: created.id_dualsim, nouvelles_valeurs: rows[i] })
              inserted++
            } catch (e: unknown) {
