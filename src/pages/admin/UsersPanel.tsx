...
export function UsersPanel() {
  ...
  const columns: Column<AppUser>[] = [
    ...
    { key: 'actif', header: 'Statut', render: r => <span className={`badge ${r.actif ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{r.actif ? 'Actif' : 'Désactivé'}</span> },
    ...
    { key: 'actions', header: '', className: 'w-20 text-right',
      render: r => (
        <div className="flex justify-end gap-1">
          ...
          <button onClick={() => setToggleTarget(r)} className={`btn-icon ${r.actif ? 'text-red-400 hover:bg-red-50 hover:text-red-600' : 'text-green-500 hover:bg-green-50 hover:text-green-700'}`} title={r.actif ? 'Désactiver' : 'Réactiver'}>
      ...
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); reset() }}
        ...
        <form id="user-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            {errors.nom_complet && <p className="mt-1 text-xs text-red-600">{errors.nom_complet.message}</p>}
          </div>
          <div>
