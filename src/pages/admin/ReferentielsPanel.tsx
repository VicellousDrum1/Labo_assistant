                    ...
                    {data.map(row => (
                      <tr key={row.id} className={`hover:bg-gray-50 transition-colors ${!row.actif ? 'opacity-50' : ''}`}>
                        ...
                        <td className="px-4 py-2.5 text-center">
                          <span className={`badge text-xs ${row.actif ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                        ...
                        <td className="px-4 py-2.5">
                          <div className="flex justify-end gap-1">
                            ...
                            <button onClick={() => toggleActif(row)} className="btn-icon" title={row.actif ? 'Désactiver' : 'Activer'}>
                              {row.actif ? <ToggleRight size={14} className="text-green-500" /> : <ToggleLeft size={14} className="text-gray-400" />}
                            </button>
                          </div>
                        </td>
                      </tr>
      ...
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); reset() }} size="sm"
        ...
        <form id="ref-form" onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          <div>
            ...
            {errors.valeur && <p className="mt-1 text-xs text-red-600">{errors.valeur.message}</p>}
          </div>
          <div>
