...
import { Plus, Pencil, Trash2, Laptop, Upload } from 'lucide-react'
...
const IMPORT_COLUMNS: ImportColumn[] = [
  ...
  { key: 'fonction',          header: 'Fonction',       required: false },
  { key: 'type_utilisation',  header: 'Type utilisation', required: false,
    validate: v => !v || TYPES_UTILISATION.includes(v) ? null : `Type invalide : "${v}"` },
...
export function PostesPage() {
  ...
  async function handleImport(rows: Record<string, unknown>[]): Promise<ImportResult> {
    ...
    const existingPostes = await fetchExistingSet(
      'poste', 'id_materiel', [...materielMap.values()].map(m => m.id_materiel)
    )
    ...
    rows.forEach((row, i) => {
      ...
      if (existingPostes.has(materiel.id_materiel)) {
        errors.push({ row: i + 2, message: `Un poste existe déjà pour "${numInv}"` }); return
      }
      ...
      validRows.push({
        ...
        payload: {
          ...
          type_utilisation: row.type_utilisation || null,
          observation: row.observation || null,
        },
      })
    })

    const { created, errorsByIndex } = await batchInsert<{ id_poste: string }>(
      'poste', validRows.map(v => v.payload), 'id_poste'
    )
    ...
    const auditEntries = validRows
      ...
      .filter(({ row }) => row)
      .map(({ v, row }) => ({
        utilisateur: user?.id ?? null, action: 'IMPORT', table_concernee: 'poste',
        id_enregistrement: row!.id_poste, nouvelles_valeurs: v.payload,
      }))
    await batchLogAudit(auditEntries)

    const inserted = created.filter(Boolean).length
    if (inserted > 0) {
      toast.success(`${inserted} poste${inserted > 1 ? 's' : ''} importé${inserted > 1 ? 's' : ''}`)
      load()
    }
    ...
    { key: 'actions', header: '', className: 'w-20 text-right',
      render: r => isAssistant ? (
        <div className="flex justify-end gap-1">
          ...
          <button onClick={() => setDeleteTarget(r)} className="btn-icon text-danger-500 hover:text-danger-700 hover:bg-danger-50" title="Supprimer"><Trash2 size={14} /></button>
        </div>
      ) : null
    },
  ]
  ...
  return (
    <div className="space-y-4 animate-fade-in">
        ...
        <form id="poste-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          {!editing && (
            <div>
              ...
              {errors.id_materiel && <p className="mt-1 text-xs text-danger-700">{errors.id_materiel.message}</p>}
            </div>
          )}
          <div>
      ...
      <ImportExcel
        ...
        title="Importer les postes depuis Excel"
