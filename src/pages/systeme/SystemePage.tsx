...
import { Monitor, Pencil, Plus, Upload } from 'lucide-react'
...
const IMPORT_COLUMNS: ImportColumn[] = [
  ...
  { key: 'statut_systeme',    header: 'Statut OS',     required: true,
...
export function SystemePage() {
  ...
  // Compteurs KPI — de vrais comptages SQL sur la table entière, indépendants
  // de la page actuellement affichée (le bug précédent calculait "Migrés"/
  // "En cours"/"Non migrés" sur les seules lignes de la page visible, d'où
  // des chiffres très inférieurs à la réalité du terrain dès que le total
  // dépassait la taille d'une page).
  const loadKpiCounts = useCallback(async () => {
    const [totalRes, migresRes, enCoursRes, nonMigresRes] = await Promise.all([
      supabase.from('systeme_exploitation').select('*', { count: 'exact', head: true }),
      supabase.from('systeme_exploitation').select('*', { count: 'exact', head: true }).eq('statut_systeme', 'Migré'),
      supabase.from('systeme_exploitation').select('*', { count: 'exact', head: true }).eq('statut_systeme', 'En cours de migration'),
      supabase.from('systeme_exploitation').select('*', { count: 'exact', head: true }).eq('statut_systeme', 'Non migré'),
    ])
    setKpiCounts({
  ...
  async function handleImport(rows: Record<string, unknown>[]): Promise<ImportResult> {
    ...
    const existingSys = await fetchExistingSet(
      'systeme_exploitation', 'id_materiel', [...materielMap.values()].map(m => m.id_materiel)
    )
    ...
    rows.forEach((row, i) => {
      ...
      if (existingSys.has(materiel.id_materiel)) {
        errors.push({ row: i + 2, message: `Un suivi OS existe déjà pour "${numInv}"` }); return
      }
      ...
      validRows.push({
        origIndex: i,
        payload: { id_materiel: materiel.id_materiel, statut_systeme: row.statut_systeme, observation: row.observation || null },
      })
    })

    const { created, errorsByIndex } = await batchInsert<{ id_systeme: string }>(
      'systeme_exploitation', validRows.map(v => v.payload), 'id_systeme'
    )
    ...
    const auditEntries = validRows
      ...
      .filter(({ row }) => row)
      .map(({ v, row }) => ({
        utilisateur: user?.id ?? null, action: 'IMPORT', table_concernee: 'systeme_exploitation',
        id_enregistrement: row!.id_systeme, nouvelles_valeurs: v.payload,
      }))
    await batchLogAudit(auditEntries)

    const inserted = created.filter(Boolean).length
    if (inserted > 0) {
      toast.success(`${inserted} suivi${inserted > 1 ? 's' : ''} OS importé${inserted > 1 ? 's' : ''}`)
      load()
      loadKpiCounts()
    }
  ...
  // KPIs — comptages globaux réels (voir loadKpiCounts), pas les seules
  // lignes de la page affichée
  const kpi = kpiCounts
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="page-header">
        ...
        <div className="flex gap-2">
          <button onClick={async () => {
            // Export complet (toutes les pages, avec les filtres actifs) —
            // pas uniquement les lignes actuellement affichées à l'écran.
            const all = await fetchAllRows<SysWithInv>(() => {
              let eq = supabase.from('systeme_exploitation')
                .select(`*, inventaire:inventaire(id_materiel,numero_inventaire,marque_modele,utilisateur,societe,exploitation)`)
                .order('date_modification', { ascending: false })
              if (filterStatut) eq = eq.eq('statut_systeme', filterStatut)
              return eq
            })
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total', value: kpi.total, color: 'bg-info-50 text-info-700' },
          { label: 'Migrés', value: kpi.migres, color: 'bg-ok-50 text-ok-700' },
          { label: 'En cours', value: kpi.enCours, color: 'bg-amber-50 text-amber-700' },
          { label: 'Non migrés', value: kpi.nonMigres, color: 'bg-danger-50 text-danger-700' },
        ].map(k => (
      ...
      <ImportExcel
        ...
        title="Importer les statuts OS depuis Excel"
        columns={IMPORT_COLUMNS}
        templateName="systeme_exploitation"
