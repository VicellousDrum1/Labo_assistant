...
const IMPORT_COLUMNS_DIDS: ImportColumn[] = [
  { key: 'type_demande',   header: 'Type (DI/DS)',     required: true,
    validate: v => ['DI','DS'].includes(v) ? null : 'Type invalide, valeurs acceptées : DI ou DS' },
  { key: 'numero_demande', header: 'N° Demande',       required: false },
  { key: 'date_demande',   header: 'Date demande',     required: false,
    transform: v => v || null },
  ...
  { key: 'objet',          header: 'Objet',            required: true },
...
export function DiDsPage() {
  ...
  async function handleImport(rows: Record<string, unknown>[]): Promise<ImportResult> {
    const { created, errorsByIndex } = await batchInsert<{ id_di_ds: string }>('suivi_di_ds', rows, 'id_di_ds')
    ...
    const auditEntries = created
      ...
      .filter(({ c }) => c)
      .map(({ c, row }) => ({
        utilisateur: user?.id ?? null, action: 'IMPORT', table_concernee: 'suivi_di_ds',
        id_enregistrement: c!.id_di_ds, nouvelles_valeurs: row,
      }))
    await batchLogAudit(auditEntries)

    const inserted = created.filter(Boolean).length
    if (inserted > 0) { toast.success(`${inserted} demande${inserted > 1 ? 's' : ''} importée${inserted > 1 ? 's' : ''}`); load() }
  ...
  const columns: Column<SuiviDiDs>[] = [
    { key: 'type_demande', header: 'Type', render: r => <span className={`badge text-xs ${r.type_demande === 'DI' ? 'bg-info-100 text-info-700' : 'bg-info-100 text-info-700'}`}>{r.type_demande}</span> },
    ...
    <div className="space-y-4 animate-fade-in">
      <div className="page-header">
        ...
        <div className="flex gap-2">
          ...
          {isAssistant && <button onClick={openCreate} className="btn-primary text-sm gap-1.5"><Plus size={14} />Nouvelle demande</button>}
        </div>
      </div>
      ...
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); reset() }}
        ...
        <form id="dids-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            {errors.objet && <p className="mt-1 text-xs text-danger-700">{errors.objet.message}</p>}
          </div>
          <div>
      ...
      <ImportExcel
        ...
        title="Importer les DI/DS depuis Excel"
